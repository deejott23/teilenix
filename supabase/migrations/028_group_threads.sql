...
CREATE INDEX group_threads_trip ON group_threads (trip_id, created_at DESC);
...
CREATE INDEX group_messages_thread ON group_messages (thread_id, created_at ASC);
CREATE INDEX group_messages_trip   ON group_messages (trip_id,   created_at DESC);

-- ── Reactions (trip_id denormalized → simple RLS) ─────────────────────────────
...
CREATE POLICY threads_insert ON group_threads FOR INSERT
  WITH CHECK (
    ...
    AND created_by_participant_id IN (
      ...
      WHERE tp.trip_id = group_threads.trip_id
...
CREATE POLICY threads_delete ON group_threads FOR DELETE
  USING (
    created_by_participant_id IN (
      SELECT tp.id FROM trip_participants tp WHERE tp.user_id = auth.uid()
    )
  );
...
CREATE POLICY messages_insert ON group_messages FOR INSERT
  WITH CHECK (
    ...
    AND participant_id IN (
      SELECT tp.id FROM trip_participants tp
      WHERE tp.trip_id = group_messages.trip_id
        AND tp.user_id = auth.uid()
        AND NOT tp.is_group
    )
  );
...
-- message_reactions — trip_id makes these trivially simple
...
CREATE POLICY reactions_insert ON message_reactions FOR INSERT
  WITH CHECK (
    ...
    AND participant_id IN (
      SELECT tp.id FROM trip_participants tp
      WHERE tp.trip_id = message_reactions.trip_id
        AND tp.user_id = auth.uid()
        AND NOT tp.is_group
    )
  );
