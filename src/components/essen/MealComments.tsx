...
import type { MealComment } from '@/types/app'
...
export default function MealComments({
  ...
  const handleDelete = async (commentId: string) => {
    ...
    try {
      await fetch(`/api/trips/${tripId}/meals/${mealId}/comments`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  const grouped: { dateLabel: string; items: MealComment[] }[] = []
  for (const c of comments) {
    const label = formatDateHeader(c.created_at)
    const last = grouped[grouped.length - 1]
    if (last && last.dateLabel === label) last.items.push(c)
    else grouped.push({ dateLabel: label, items: [c] })
  }
